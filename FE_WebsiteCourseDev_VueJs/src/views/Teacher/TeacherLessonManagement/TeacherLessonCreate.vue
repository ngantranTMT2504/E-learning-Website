<template>
  <div>
    ...
    <!-- Title -->
    <h2 class="fw-bold text-uppercase mb-3">Thêm bài học mới</h2>
    ...
    <div class="card shadow-sm">
      <div class="card-body p-4">
        ...
        <div class="text-end mt-4">
          <a @click="submit">
            <ButtonCustom><i class="bi bi-save me-2"></i> Thêm bài học</ButtonCustom>
          </a>
        </div>
      </div>
    </div>
...
const courseId = route.query.courseId ?? null
const accountId = 4
...
onMounted(async () => {
  try {
    ...
    console.log(courses.value);
     // Nếu có courseId trong query → gán vào form
    if (courseId) {
      const matchedCourse = courses.value.find(c => c.id == courseId)
      if (matchedCourse) {
        lesson.courseId = matchedCourse.id
      }
    }
  } catch (error) {
